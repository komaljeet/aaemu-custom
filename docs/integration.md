...
2. **Boss respawn poll** → `GetBossesReadyToSpawnAsync` polled by a spawn tick that calls the boss's `NpcSpawner.ForceSpawn(0)` (spawner lookup by `NpcSpawnerNpc.MemberId == boss_id`; spawn position lives on the spawner's `WorldSpawnPosition`, JSON-backed not DB-backed). **Deferred by decision (2026-07-16):** AAEmu already respawns bosses natively (`NpcSpawner.DoDespawn` schedules a native respawn on every death), so a sidecar poll would double-spawn unless native respawn is suppressed for sidecar-acked kills. The chosen scope was kill-loot only — native respawn stays authoritative for now. The sidecar's per-minute `tick_boss_spawns` was fixed to be **log-only** (it previously cleared `next_spawn_at = NULL`, which would erase the ready signal before any poll could read it); `boss_spawn_state` is now preserved for when the poll is wired. Wiring the poll later requires the authority decision: (a) sidecar-authoritative (suppress native respawn for sidecar-acked kills, with native fallback when the sidecar is down at kill time) + a liveness registry to avoid double-spawn on confirm failure + a new `POST /boss/spawned` confirm endpoint; or (b) a non-suppressing safety-net poll that only spawns bosses no live instance of which exists and that aren't already natively scheduled.
